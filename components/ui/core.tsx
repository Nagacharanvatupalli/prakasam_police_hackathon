...
export function RiskBadge({ level = "verified", className, score }: RiskBadgeProps) {
  ...
  const normalizedLevel = typeof level === "string"
    ? level.trim().toLowerCase().replace(/\s+/g, "_")
    : "";
  const fallbackMap: Record<string, RiskLevel> = {
    low_confidence: "suspicious",
    low: "safe",
    watchlist: "high_risk",
    unknown: "verified",
  };
  const safeLevel = (config[normalizedLevel as RiskLevel] as { label: string; classes: string; pulse: boolean } | undefined)
    ? (normalizedLevel as RiskLevel)
    : fallbackMap[normalizedLevel] ?? "verified";
  const { label, classes, pulse } = config[safeLevel];
